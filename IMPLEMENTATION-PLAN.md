...
| 五个角色（加入学访谈的学习顾问、独立的提案评审员） | 系统提示片段（`before_agent_start` 追加）+ `pi.setActiveTools` 白名单 | `roles.ts`、`index.ts` |
