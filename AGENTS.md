...
- `library/` 学习者的本地资料副本与 Zotero 导入文件（`/collect` 下载或登记），不进版本库
...
- `proposals/` 规划、资料与整理提案，学习者用 `/accept` 接受后才写入正式文件；已接受的提案改名为 `*.accepted.json`；`/critique` 的评审写在同名 `*.review.json` / `*.review.md`
...
`/learn` 概览 · `/placement [n]` 水平测试（画像 + 诊断） · `/plan [replan|revise]` 规划 · `/critique [文件]` 独立评审提案 · `/exemplar <名字>` 提供范例 · `/accept` 接受提案 · `/sources [unit] [障碍说明]` 选材 · `/collect [id]` 获取与入库 · `/verify [id]` 标记资料已核验 · `/curate [unit]` 整理馆藏 · `/library [unit]` 馆藏概览 · `/read [unit]` 阅读会话 · `/hint` `/explain` 切换模式 · `/answer` 闭卷作答 · `/gloss <id>` 写术语表 · `/done` 结束会话 · `/artifact <名字>` 写产出物 · `/review <文件> [unit]` 评审 · `/assess [n]` 出题 · `/take` 作答（复盘测试或水平测试） · `/reflect` 写复盘 · `/events` `/dispatch` 事件 · `/role <name|none>` 原地切换角色
